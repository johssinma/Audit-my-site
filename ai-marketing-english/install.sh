...
# Installs English skills, agents and scripts into Claude Code
...
echo ""
echo -e "${CYAN}╔══════════════════════════════════════════════╗${NC}"
echo -e "${CYAN}║        Audit My Site                         ║${NC}"
echo -e "${CYAN}║        Complete AI Marketing Suite           ║${NC}"
echo -e "${CYAN}║        English Version                       ║${NC}"
...
if [ -n "$BASH_SOURCE" ] && [ "$BASH_SOURCE" != "bash" ] && [ -f "$BASH_SOURCE" ]; then
    ...
    SCRIPT_DIR="$TEMP_DIR/audit-my-site/ai-marketing-english"
fi
...
# Install main orchestrator skill
...
for agent in "${AGENTS[@]}"; do
    if [ -f "$SCRIPT_DIR/agents/$agent.md" ]; then
        ...
        echo -e "  ${YELLOW}⚠${NC} $agent (not found, skipped)"
    fi
done
...
if command -v python3 &>/dev/null; then
    ...
    if python3 -c "import reportlab" 2>/dev/null; then
        ...
        echo -e "  ${YELLOW}⚠${NC} reportlab not installed (required for PDF reports)"
    ...
    echo -e "  ${YELLOW}⚠${NC} Python 3 not found — scripts will not work"
...
echo "  /market quick <url>        60-second marketing overview"
echo "  /market copy <url>         Generate optimised copywriting"
echo "  /market emails <topic>     Generate email sequences"
echo "  /market social <topic>     Social media content calendar"
echo "  /market ads <url>          Ad creatives & copy"
echo "  /market funnel <url>       Sales funnel analysis"
echo "  /market competitors <url>  Competitive intelligence"
echo "  /market landing <url>      Landing page CRO analysis"
echo "  /market launch <product>   Product launch plan"
...
echo "  /market seo <url>          Content SEO audit"
